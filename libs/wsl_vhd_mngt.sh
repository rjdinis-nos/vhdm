...
# Source tracking file management functions
# All tracking file operations are in a separate module for better organization
if [[ -f "$SCRIPT_DIR/wsl_vhd_tracking.sh" ]]; then
    source "$SCRIPT_DIR/wsl_vhd_tracking.sh"
fi
...
wsl_find_uuid_by_path() {
    ...
    local tracked_uuid=$(tracking_file_lookup_uuid_by_path "$vhd_path_win")
...
wsl_create_vhd() {
    ...
    tracking_file_save_mapping "$vhd_path_win" "$new_uuid" "" "$new_dev"
